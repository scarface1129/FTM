<?php
    ...
    // $UserFullName = adminGetSpecificUser($conn, $userId);
...
<body>
  ...
  <section class="compass admin-compass user-compass">
    ...
    <?php
    if (adminGetUnconfirmedDeposits($conn) === false) {
        ?>
         <div class="no-user-contain">
        <div class="no-user-icon">
        </div>
        <p>
          No deposits to confirm.
        </p>
      </div>
        <?php
    } else {
        $row = adminGetUnconfirmedDeposits($conn);   
    ?>
            <?php foreach($row as $users) {?>
      <a href="confirm-chosen-deposit.php?depositId=<?php echo $users['NoId'];?>">
                   <div class="user">
