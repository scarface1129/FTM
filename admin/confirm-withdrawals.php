...
<body>
  ...
  <section class="compass admin-compass user-compass">
    ...
    <?php
    if (adminGetUnconfirmedWithdrawals($conn) === false) {
        ...
        <p>
          No withdrawals to confirm.
        </p>
      </div>
        <?php
    } else {
        $row = adminGetUnconfirmedWithdrawals($conn);   
    ?>
            <?php foreach($row as $users) {?>
      <a href="confirm-chosen-withdrawal.php?withdrawalId=<?php echo $users['NoId'];?>">
                   <div class="user">
