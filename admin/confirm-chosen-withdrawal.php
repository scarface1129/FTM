<?php
   ...
   if (getSpecificWithdrawal($conn, $withdrawalId) == false) {
    header("Location: confirm-withdrawal.php?msg=withdrawalDoesNotExist");
    exit();
   } 
   ...
   if (adminGetSpecificUser($conn, $userId) == false) {
    header("Location: confirm-withdrawal.php?msg=userDoesNotExist");
    exit();
   }

   $user = adminGetSpecificUser($conn, $userId);
   $withdrawal_status = "Unconfirmed";
   // withdrawalDate
   

   $latestWithdrawal = getLatestWithdrawal($conn, $userId, $withdrawal_status);
   
   if ($latestWithdrawal == false) {
       header("Location: confirm-withdrawal.php?msg=noLatestWithdrawal");
       exit();
   }
...
<body>
    ...
    <form method="GET" action="allow-withdrawal.php">
            ...
            <div class="quick-tasks user-quick-task-desktop-flex">
                ...
                <div class="quick-tasks-sub-flex-change">
                    <div class="quick-tasks-sub">
                        <p class="tab-title">Confirm Withdrawal</p>
                    </div>
                    <div class="value-withdraw-deposit">
                        <div class="value-amount-withdraw-deposit-contain">
                            <p>Amount</p>
                            <p class="value-amount-withdraw-deposit">
                                $<?php echo number_format($latestWithdrawal['withdrawalAmt'], 2); ?></p>
                        </div>
                        <div class="value-amount-withdraw-deposit-contain">
                            <p>Wallet Address</p>
                            <div class="wallet-address-value">
                                <p><?php echo $withdrawal['withdrawalWallet']; ?></p>
                            </div>
                        </div>
                        <div class="value-amount-withdraw-deposit-contain">
                            <p>Cryptocurrency</p>
                            <div class="wallet-address-value">
                                <p><?php echo $withdrawal['withdrawalCurrency']; ?></p>
                            </div>
                        </div>
                        <div class="button-admin-area">
                            <input name="withdrawalId" type="hidden" value="<?php echo $withdrawal['NoId']; ?>">
                            <input name="withdrawalWallet" type="hidden" value="<?php echo $withdrawal['withdrawalWallet']; ?>">
                            <input name="amt" type="hidden" value="<?php echo $withdrawal['withdrawalAmt']; ?>">
                            <button class="inv-btn payout-btn" type="submit" name="allow-withdrawal" value="Allow">
                                Allow
                            </button>
                        </div>
                    </div>
